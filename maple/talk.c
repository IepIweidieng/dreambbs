    ...
    int fd)
{
    ...
    for (;;)
    {
        ...
        if (ch == I_OTHERDATA)
        {
            ...
            if (data[0] == Ctrl('A'))
            { /* Thor.990219: 呼叫外掛棋盤 */
                if (DL_func(DL_NAME("bwboard.so", vaBWboard), fd, 1)==-2)
                    break;
                continue;
            }
            if (data[0] == Ctrl('B'))
            {
                if (DL_func(DL_NAME("chess.so", vaChess), fd, 1)==-2)
                    break;
                continue;
            }
#endif
        ...
        { /* Thor.990219: 呼叫外掛棋盤 */
            ...
            if (send(fd, data, 1, 0) != 1)
                break;
            /* if (BWboard(fd, 0)==-2) */
            if (DL_func(DL_NAME("bwboard.so", vaBWboard), fd, 0)==-2)
                break;
        }
        else if (ch == Ctrl('B'))
        { /* Thor.990219: 呼叫外掛棋盤 */
            ...
            if (send(fd, data, 1, 0) != 1)
                break;
            /* if (BWboard(fd, 0)==-2) */
            if (DL_func(DL_NAME("chess.so", vaChess), fd, 0)==-2)
                break;
        }
#endif
