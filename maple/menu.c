...
static MENU menu_boardadm[] =
{
    ...
    {DL_NAME("adminutil.so", bm_check), PERM_BOARD|PERM_SYSOP, - M_XMODE,
    "Manage     板主確認"},

    {DL_NAME("adminutil.so", m_expire), PERM_BOARD|PERM_SYSOP, - M_XMODE,
    "DExpire    清除看板刪除文章"},

    {DL_NAME("adminutil.so", mail_to_bm), PERM_SYSOP, - M_XMODE,
    "ToBM       寄信給板主"},

    {DL_NAME("adminutil.so", mail_to_all), PERM_SYSOP, - M_XMODE,
    "Alluser    系統通告"},

    {DL_NAME("personal.so", personal_admin), PERM_BOARD|PERM_SYSOP, - M_XMODE,
    "Personal   個人板審核"},
...
static MENU menu_accadm[] =
{
    ...
    {DL_NAME("bank.so", money_back), PERM_ACCOUNTS, - M_XMODE,
    "GetMoney   匯入舊夢幣"},

#ifdef  HAVE_SONG
    {DL_NAME("song.so", AddRequestTimes), PERM_KTV, - M_XMODE,
    "Addsongs   增加點歌次數"},
#endif

    {DL_NAME("passwd.so", new_passwd), PERM_SYSOP, - M_XMODE,
    "Password   重送新密碼"},
    ...
    {DL_NAME("observe.so", Observe_list), PERM_SYSOP|PERM_BOARD, - M_XMODE,
    "2Observe   系統觀察名單"},
#endif
...
static MENU menu_settingadm[] =
{
    ...
    {DL_NAME("adminutil.so", m_xfile), PERM_SYSOP, - M_XFILES,
    "File       編輯系統檔案"},

    {DL_NAME("adminutil.so", m_xhlp), PERM_SYSOP, - M_XFILES,
    "Hlp        編輯說明檔案"},

    {DL_NAME("admin.so", Admin), PERM_SYSOP, - M_XMODE,
    "Operator   系統管理員列表"},

    {DL_NAME("chatmenu.so", Chatmenu), PERM_CHATROOM|PERM_SYSOP, - M_XMODE,
    "Chatmenu   " CHATROOMNAME "動詞"},

    {DL_NAME("violate.so", Violate), PERM_SYSOP, - M_XMODE,
    "Violate    處罰名單"},

    {DL_NAME("adminutil.so", special_search), PERM_SYSOP, - M_XMODE,
    "XSpecial   特殊搜尋"},

    {DL_NAME("adminutil.so", update_all), PERM_SYSOP, - M_XMODE,
    "Database   系統資料庫更新"},
...
{
    {DL_NAME("adminutil.so", reset1), PERM_BOARD, - M_XMODE,
    "Camera     動態看板"},

    {DL_NAME("adminutil.so", reset2), PERM_BOARD, - M_XMODE,
    "Group      分類群組"},

    {DL_NAME("adminutil.so", reset3), PERM_SYSOP, - M_XMODE,
    "Mail       寄信收信轉信"},

    {DL_NAME("adminutil.so", reset4), PERM_ADMIN, - M_XMODE,
    "Killbbs    清除不正常 BBS"},

    {DL_NAME("adminutil.so", reset5), PERM_BOARD, - M_XMODE,
    "Firewall   擋信列表"},

    {DL_NAME("adminutil.so", reset6), PERM_CHATROOM, - M_XMODE,
    "Xchatd     重開聊天室"},

    {DL_NAME("adminutil.so", reset7), PERM_SYSOP, - M_XMODE,
    "All        全部"},
...
static MENU menu_admin[] =
{
    ...
    {DL_NAME("innbbs.so", a_innbbs), PERM_BOARD, - M_SYSTEM,
    "InnBBS     轉信設定"},
...
static MENU menu_mail[] =
{
    ...
    {DL_NAME("contact.so", Contact), PERM_INTERNET, - M_XMODE,
    "Contact    聯絡名單"},
...
static MENU menu_talk[] =
{
    ...
    "Banmsg     拒收訊息名單"},
#endif
    {DL_NAME("aloha.so", t_aloha), PERM_BASIC, - M_XMODE,
    "Aloha      上站通知名單"},
    ...
    {DL_NAME("chat.so", t_chat), PERM_CHAT, - M_CHAT,
...
static MENU menu_xyz[] =
{
    ...
    {DL_NAME("xyz.so", x_siteinfo), 0, - M_READA,
    "Xinfo      系統程式資訊"},
...
static MENU menu_reg[] =
{
    ...
    {DL_NAME("list.so", List), PERM_VALID, - M_XMODE,
    "1List      群組名單"},
...
static MENU menu_user[] =
{
    ...
    {DL_NAME("memorandum.so", Memorandum), PERM_VALID, -M_XMODE,
    "Note       備忘錄"},

    {DL_NAME("pnote.so", main_note), PERM_VALID, - M_XMODE,
    "PNote      個人答錄機"},

#ifdef  HAVE_CLASSTABLEALERT
    {DL_NAME("classtable2.so", main_classtable), PERM_VALID, -M_XMODE,
    "2Table     新版個人功\課表"},
#endif
...
{
    {DL_NAME("bj.so", BlackJack), PERM_VALID, - M_XMODE,
    "BlackJack  " NICKNAME "黑傑克"},

    {DL_NAME("guessnum.so", fightNum), PERM_VALID, - M_XMODE,
    "FightNum   數字大決戰"},

    {DL_NAME("guessnum.so", guessNum), PERM_VALID, - M_XMODE,
    "GuessNum   傻瓜猜數字"},

    {DL_NAME("mine.so", Mine), PERM_VALID, - M_XMODE,
    "Mine       " NICKNAME "踩地雷"},

    {DL_NAME("pip.so", p_pipple), PERM_VALID, - M_XMODE,
    "Pip        " NICKNAME "戰鬥雞"},
...
static MENU menu_special[] =
{
    ...
    {DL_NAME("personal.so", personal_apply), PERM_VALID, - M_XMODE,
    "PBApply      申請個人看板"},

    {DL_NAME("bank.so", bank_main), PERM_VALID, - M_XMODE,
    "Bank       　銀行"},

    {DL_NAME("shop.so", shop_main), PERM_VALID, - M_XMODE,
    "Pay        　商店"},
...
static MENU menu_song[] =
{
    {DL_NAME("song.so", XoSongMain), PERM_VALID, - M_XMODE,
    "Request       點歌歌本"},

    {DL_NAME("song.so", XoSongLog), PERM_VALID, - M_XMODE,
    "OrderSongs    點歌紀錄"},

    {DL_NAME("song.so", XoSongSub), PERM_VALID, - M_XMODE,
    "Submit        投稿專區"},
...
/* 091007.cache: 拉人灌票沒意義... */
    ...
    {DL_NAME("newboard.so", XoNewBoard), PERM_VALID, - M_XMODE,
    "Cosign    【 連署申請區 】"},

    {DL_NAME("vote.so", SystemVote), PERM_POST, - M_XMODE,
    "Vote      【 系統投票區 】"},
