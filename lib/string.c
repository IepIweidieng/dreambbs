 ...
 * It behaviors like `strlcpy`, except that it does not read more than `siz` bytes from `src`.
 * It behaviors the same as `strscpy` from the Linux Kernel API, except that it return `-1` when truncation occurs.
 */
GCC_NONNULLS
